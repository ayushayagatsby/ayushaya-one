...
const options = {
    ...
    renderNode: {
        ...
        [BLOCKS.HEADING_1]: (node, children) => {
            return (
                <h3 css={css `margin: 5% 0 0 0;`}>{children}</h3>
            )
        },
        [BLOCKS.HEADING_3]: (node, children) => {
            return (
                <h4 css={css `margin-bottom: 3%;`}>{children}</h4>
            )
        },
            ...
            <ProgressBarContainer id="blog-bar">
                ...
                <div css={css `display: flex; align-items: center; &>*{margin-right: 20px;}`}>
                    <Link to="/publications">
                        <div
                            css={css `display: flex; justify-content: center; align-items: center; width: 10vw; height: 10vw; background-color: #F9F5F1; border-radius: 50%; margin-left: 10px;`}>
